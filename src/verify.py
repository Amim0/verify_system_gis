...
class IntegrityVerifier(Verifier):
    ...
    def verify(self, manifest: dict, model) -> tuple:
        h1 = manifest.get("weights_hash", "")
        h2 = self.get_weights_hash(model)
        return h1 == h2, h1, h2
...
class ReproducibilityVerifier(Verifier):
    ...
    def verify(self, model_export, refs) -> tuple:
...
# =========================================================
# ОРКЕСТРАТОР (твой бывший VerifyApp)
# =========================================================
...
class VerifyApp:
    ...
    def __init__(self):
        ...
        self.repro_verifier = ReproducibilityVerifier()
    ...
    def verify(self):
        ...
        sig_ok = self.signature_verifier.verify(manifest)

        print("Проверка целостности...")
        integrity_ok, h1, h2 = self.integrity_verifier.verify(
            manifest, model_exp
        )

        print("Проверка воспроизводимости...")
        repro_score, samples = self.repro_verifier.verify(
            model_exp, refs
        )
